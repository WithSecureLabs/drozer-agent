...
import com.WithSecure.jsolar.api.builders.ReflectionResponseFactory;
import com.WithSecure.jsolar.api.sessions.Session;
import com.WithSecure.jsolar.api.Protobuf.Message;
import com.WithSecure.jsolar.reflection.Reflector;
import com.WithSecure.jsolar.reflection.types.ReflectedType;

import java.util.List;

// Don't even bother attempting to understand this, all it does is convert from protobufs to our reflected types
// Its a whole bunch of java mess
public class ReflectionMessageHandler implements MessageHandler{
    ...
    private Message createResponse(Message request, ReflectionResponseFactory responseFactory) {
        return new MessageFactory(responseFactory.setSessionId(this.session.getSessionId())).inReplyTo(request).build();
    }
